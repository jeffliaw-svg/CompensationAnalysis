...
.selected-bar .sel-label { font-weight: 700; min-width: 90px; }
.selected-bar .sel-stat { display: flex; gap: 16px; flex-wrap: wrap; }
.selected-bar .sel-item { font-size: 13px; }
.selected-bar .sel-item .sel-emp { opacity: 0.7; margin-right: 4px; }
.selected-bar .sel-item .sel-val { font-weight: 700; }
...
function updateSelection() {
  ...
  var label = document.getElementById('selLabel');
  ...
  label.textContent = selLocs.length + ' selected';
  var fields = [
    {key: 'walmart', label: 'WMT'},
    {key: 'homedepot', label: 'HD'},
    {key: 'costco', label: 'COST'},
    {key: 'starbucks', label: 'SBUX'},
    {key: 'blended', label: 'Blended'}
  ];
  var html = '';
  fields.forEach(function(f) {
    var s = computeStats(selLocs, f.key, role);
    var val = s ? '$' + s.mean.toFixed(2) : '—';
    html += '<span class="sel-item"><span class="sel-emp">' + f.label + ':</span><span class="sel-val">' + val + '</span></span>';
  });
