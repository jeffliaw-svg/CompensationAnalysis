...
th.col-wage, td.col-wage { text-align: right; }
td { padding: 7px 10px; border-bottom: 1px solid var(--gray-200); }
tr:hover td { background: var(--gray-100); }
col.col-chk { width: 36px; }
col.col-q { width: 44px; }
col.col-yard { width: 22%; }
col.col-city { width: 13%; }
col.col-emp { width: 11%; }
col.col-blend { width: 10%; }
...
th.col-chk { width: 36px; text-align: center; cursor: default; }
th.col-chk:hover { background: var(--navy); }
td.col-chk { text-align: center; }
td.col-chk input[type="checkbox"] { cursor: pointer; }

tr.selected-row { display: none; }
tr.selected-row.visible { display: table-row; }
tr.selected-row td {
  background: var(--gray-200) !important;
  color: var(--navy);
  font-weight: 700;
...
<div class="container">
  ...
  <div class="section">
    ...
    <div id="tab-locations" class="tab-content active">
      ...
      <div class="legend">
        <span class="legend-item">Highest <span class="q-badge q1">Q1</span></span>
        <span class="legend-item"><span class="q-badge q2">Q2</span></span>
        <span class="legend-item"><span class="q-badge q3">Q3</span></span>
        <span class="legend-item">Lowest <span class="q-badge q4">Q4</span></span>
      ...
      <div class="table-wrap">
        <table id="locTable">
          ...
          <thead>
            <tr>
              <th class="col-chk"><input type="checkbox" id="selectAll" onchange="toggleSelectAll(this)" title="Select all visible"></th>
              ...
              <th class="col-wage" data-col="walmart" onclick="sortTable('walmart')">Walmart <span class="sort-arrow">&#9650;</span></th>
              <th class="col-wage" data-col="homedepot" onclick="sortTable('homedepot')">Home Depot <span class="sort-arrow">&#9650;</span></th>
              <th class="col-wage" data-col="costco" onclick="sortTable('costco')">Costco <span class="sort-arrow">&#9650;</span></th>
              <th class="col-wage" data-col="starbucks" onclick="sortTable('starbucks')">Starbucks <span class="sort-arrow">&#9650;</span></th>
              <th class="col-wage" data-col="blended" onclick="sortTable('blended')">Blended <span class="sort-arrow">&#9650;</span></th>
            </tr>
          </thead>
...
function blendedCell(loc, role) {
  ...
  return '<td class="wage-cell col-wage"><strong>$' + val.toFixed(2) + '</strong>' +
...
function empCell(emp, empKey, loc, role) {
  ...
  if (e && isInRange(loc, shortKey)) {
    return '<td class="wage-cell col-wage">' + fmtWageWithDist(e.hourly_avg, e.source_url, e.source_name, dist[shortKey] || 0) + '</td>';
  }
  return '<td class="na-cell col-wage" title="Nearest ' + emp + ': ' + (dist[shortKey] || 0).toFixed(1) + ' mi (>' + CUTOFF + ' mi cutoff)">N/A</td>';
}
...
function renderTable() {
  ...
  var html = '';
  html += '<tr class="selected-row" id="selectedRow">';
  html += '<td class="col-chk"></td>';
  html += '<td></td>';
  html += '<td><strong class="sel-count">Composite</strong></td>';
  html += '<td></td>';
  html += '<td class="col-wage" data-sel="walmart"></td>';
  html += '<td class="col-wage" data-sel="homedepot"></td>';
  html += '<td class="col-wage" data-sel="costco"></td>';
  html += '<td class="col-wage" data-sel="starbucks"></td>';
  html += '<td class="col-wage" data-sel="blended"></td>';
  html += '</tr>';
  var qGroups = {};
  var cols = 9;
  ...
  locs.forEach(function(loc, idx) {
    ...
    var chk = selectedZips[loc.zip] ? ' checked' : '';

    html += '<tr' + (isFirst ? ' class="q-first"' : '') + '>';
    html += '<td class="col-chk"><input type="checkbox" data-zip="' + loc.zip + '" onchange="toggleSelect(this)"' + chk + '></td>';
    ...
    if (!nextLoc || nextLoc.quartile !== loc.quartile) {
      ...
      html += '<td class="col-wage">' + statCell(group, 'walmart', role) + '</td>';
      html += '<td class="col-wage">' + statCell(group, 'homedepot', role) + '</td>';
      html += '<td class="col-wage">' + statCell(group, 'costco', role) + '</td>';
      html += '<td class="col-wage">' + statCell(group, 'starbucks', role) + '</td>';
      html += '<td class="col-wage"><strong>' + statCell(group, 'blended', role) + '</strong></td>';
      html += '</tr>';
      html += '<tr class="subtotal-row median-row">';
      html += '<td class="col-chk"></td>';
      html += '<td></td>';
      html += '<td colspan="2"><strong>Median</strong></td>';
      html += '<td class="col-wage">' + statCellMed(group, 'walmart', role) + '</td>';
      html += '<td class="col-wage">' + statCellMed(group, 'homedepot', role) + '</td>';
      html += '<td class="col-wage">' + statCellMed(group, 'costco', role) + '</td>';
      html += '<td class="col-wage">' + statCellMed(group, 'starbucks', role) + '</td>';
      html += '<td class="col-wage"><strong>' + statCellMed(group, 'blended', role) + '</strong></td>';
      html += '</tr>';
      if (nextLoc) {
...
function toggleSelect(cb) {
  var zip = cb.getAttribute('data-zip');
  if (cb.checked) { selectedZips[zip] = true; } else { delete selectedZips[zip]; }
  updateSelection();
  updateSelectAllState();
}

function toggleSelectAll(cb) {
  var boxes = document.querySelectorAll('#locBody input[type="checkbox"]');
  boxes.forEach(function(b) {
    b.checked = cb.checked;
    var zip = b.getAttribute('data-zip');
    if (cb.checked) { selectedZips[zip] = true; } else { delete selectedZips[zip]; }
  });
  updateSelection();
}
...
function updateSelection() {
  ...
  if (!row) return;
  var zips = Object.keys(selectedZips);
  if (zips.length === 0) {
    row.classList.remove('visible');
    return;
  }
  var selLocs = DATA.locations.filter(function(l) { return selectedZips[l.zip]; });
  var fields = ['walmart', 'homedepot', 'costco', 'starbucks', 'blended'];
  var cells = row.querySelectorAll('td[data-sel]');
  cells.forEach(function(td) {
    var f = td.getAttribute('data-sel');
    var s = computeStats(selLocs, f, role);
    td.innerHTML = s ? '<strong>$' + s.mean.toFixed(2) + '</strong>' : '&mdash;';
  });
  var lbl = row.querySelector('.sel-count');
  if (lbl) lbl.textContent = 'Composite (' + selLocs.length + ' selected)';
